...
B=litao.textdata(2:size(litao.textdata,1),7:7);%纬度deg
B_rad=zeros(size(B,1),1);
for i=1:size(B,1)
    B_rad(i,1)=str2double(cell2mat(B(i,1)))/180*pi;
end
L=litao.textdata(2:size(litao.textdata,1),9:9);%经度deg
L_rad=zeros(size(L,1),1);
for i=1:size(L,1)
    L_rad(i,1)=str2double(cell2mat(L(i,1)))/180*pi;
end
H=litao.textdata(2:size(litao.textdata,1),14:14);%高程m
...
%时间偏差
for take_off_stamp = -2.5:0.1:2.5
